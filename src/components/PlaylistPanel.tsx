...
const PlaylistPanel: React.FC<PlaylistPanelProps> = ({ currentProject, onSelectProject, onClose }) => {
  ...
  if (!currentProject) {
    return (
      <div className="fixed right-[80px] top-1/2 -translate-y-1/2 w-[523px] max-h-[90vh] z-50 flex flex-col pointer-events-none animate-slideInRight">
  ...
  // --- RENDER: DETAIL VIEW (NOW PLAYING) ---
  return (
    <div className="fixed right-[80px] top-1/2 -translate-y-1/2 w-[523px] max-h-[90vh] z-50 flex flex-col gap-5 pointer-events-none animate-slideInRight">
      
      {/* TOP CARD: Layout Fix Applied - Header inside */}
      ...
      {/* BOTTOM CARD */}
      <div className="bg-white/95 backdrop-blur-md rounded-[20px] shadow-2xl p-[30px] min-h-[250px] pointer-events-auto flex flex-col">
        {viewMode === 'playingNext' ? (
                ...
                <div className="flex flex-col gap-2.5 overflow-y-auto max-h-[250px] pr-1">
                    {PROJECTS.filter(p => p.id !== currentProject.id).map(p => (
                        <div 
                            key={p.id} 
                            onClick={() => onSelectProject(p.id)}
                            className="flex items-center gap-3 p-2.5 bg-white rounded-xl border border-transparent hover:bg-purple-600/5 hover:border-purple-600/10 transition-all cursor-pointer"
                        >
            ...
            <>
                ...
                <div className="text-sm leading-relaxed text-gray-600 animate-fadeInDown">
                    {tab === 'overview' && currentProject.overview}
                    {tab === 'landing' && "Landing page content details would appear here. This section showcases the visual design structure."}
                    {tab === 'prototype' && "Interactive prototype walkthrough. Click here to view the figma prototype or live demo."}
                </div>
            </>
        )}
      </div>

       {/* Floating Close Button for the entire LP view */}
       <button 
         onClick={onClose}
         className="absolute -top-10 right-0 w-8 h-8 flex items-center justify-center bg-white/80 rounded-full text-black hover:bg-white hover:scale-110 transition-all pointer-events-auto shadow-md"
       >
         ✕
       </button>
    </div>
